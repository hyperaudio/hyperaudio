...
import InviteTeamMemberDialog from './ofRolesTable/InviteTeamMember';
import { getComparator, stableSort } from '../utils';

const PREFIX = `RolesTable`;
const classes = {
  root: `${PREFIX}-Root`,
...
export function RolesTable(props) {
  const { organization, team, account, onDeleteMembers, onChangeMemberRole, onAddMember } = props;
  const { members } = team;
  ...
  const flatMembers = members.map(({ displayName, userId, userStatus, email, payment, roles }) => ({
    displayName,
    email,
      ...
      <Root className={classes.root}>
        <Toolbar disableGutters>
          {selected.length > 0 ? (
            ...
            <Box sx={{ flex: '1 1 auto', display: 'flex', alignItems: 'center' }}>
              <Typography component="div" id="tableTitle" sx={{ mr: 2 }} variant="h6">
                All members
              </Typography>
              <Button color="primary" onClick={() => setAdding(true)} size="small" startIcon={<PersonAddIcon />}>
                Add new
              </Button>
            </Box>
          )}
            ...
            <TableBody>
              {stableSort(flatMembers, getComparator(order, orderBy))
                ...
                .map((o, index) => {
                  ...
                  return (
                    <TableRow hover key={o.userId} selected={isItemSelected} tabIndex={-1}>
                      <TableCell padding="checkbox">
                        <Checkbox
                          color="primary"
                          ...
                          onClick={event => handleSelectClick(event, o.userId)}
                          inputProps={{
      ...
      {Boolean(focused) && (
        ...
        >
          {getRoles(focused).map(role => {
            ...
            return (
              ...
              >
                ...
                <ListItemText id={role.key} primary={role.label} primaryTypographyProps={{ color: 'primary' }} />
              </MenuItem>
            );
          })}
          <Divider />
          ...
          >
            ...
            <ListItemText primaryTypographyProps={{ color: 'error' }}>Remove from team</ListItemText>
          </MenuItem>
        </Menu>
      )}
      {adding && <InviteTeamMemberDialog onClose={() => setAdding(false)} onSubmit={onAddMember} />}
    </>
  );
}
