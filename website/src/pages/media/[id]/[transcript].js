...
const MediaPage = initialData => {
  ...
  useEffect(() => {
    ...
    window.location = `/media/${id}/${t.id}`;
    // router.push(
    //   {
    //     // pathname: `/media/${id}`,
    //     pathname: `/media/${id}/${t.id}`,
                  ...
                  <div className={classes.titlesFt}>
                    {transcripts?.length > 0 && (
                      <TextField
                        ...
                        onChange={e => gotoTranscript(e.target.value)}
                        select
                        value={transcript}
                        variant="outlined"
                        SelectProps={{
              ...
              >
                ...
                <Hidden mdUp>
                  {transcripts?.length > 0 && (
                    ...
                    >
                      {transcripts.map(({ id, title, lang, status, statusMessage }) => (
                        <MenuItem key={id} selected={id === transcript} value={id}>
